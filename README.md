...
To order PCB's (from PCBWay for example) download the "production files" map of the PCB you want, the downloaded map will contain all the files you will need, that is both gerber and drill files, but most often the PCB manufacturor will want all these files in a single .ZIP file, so you have to zip up all the files in the map into a single.ZIP file first.
