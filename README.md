...
The canonical ZX81 hardware/software hobby designer Wilf Rigter designed a highresolution system for the ZX-81 in 1986 and wrote about it here: https://archive.org/details/zx-appeal/ZX-Appeal%20Jan%2086/page/12/mode/2up?view=theater in this article he stated that for it to work a ZX-81 needed an extra 8K RAM at 8K to 16L, and a Z80 that supported 8-bit refresh, while most Z80's at the time only supported 7-bit refresh counters (7-refresh address lines). I'm not sure whether modern (CMOS) Z80's support 8-bit refresh, but also the ZX81+38 (when wired for 8K ROMS space) does have RAM at 8 to 16K, but in this case it is a copy of RAM that is used elesewhere, in any case it seems that currently revion 1.9 of my ZX81+38 does not supports wilf rigters WRX16 high resolution solution. And trying to use a game based on WRX16 currently crashes.

But do note that other (pseudo) high resolution games that work on an unmodified ZX-81 do work, examples are manic Miner for the ZX-81, and many others.
